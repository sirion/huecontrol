...
export default class Popup extends HTMLElement {
	...
	async close(cancel = false) {
		const domPopup = this._shadow.querySelector(".popup");

		const onClose = () => {
			domPopup.removeEventListener("transitionend", onClose);
		...
		domPopup.addEventListener("transitionend", onClose);
		domPopup.style.opacity = "0";
	}
...
customElements.define("mi-popup", Popup);
